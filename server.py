...
אם המילה תקינה (כולל אם זה שם פרטי עברי, מילת סלנג מקובלת, או נטייה תקנית) - השב/השיבי אך ורק במילה אחת:
VALID

אם יש בה שגיאת כתיב - השב/השיבי בפורמט הבא בדיוק, שתי שורות בלבד:
CORRECT: <כאן רק המילה המתוקנת, מילה אחת בלבד, בלי שום טקסט נוסף באותה שורה>
TIP: <כאן הטיפ הקצר, בשורה נפרדת>

חשוב מאוד: בשורת CORRECT יש לכתוב אך ורק את המילה המתוקנת עצמה, ללא הסברים. ההסבר שייך אך ורק לשורת TIP.
...
"""
    try:
        ...
        if result.strip().startswith("VALID"):
            return {"valid": True}
        correct_match = re.search(r'CORRECT:\s*(\S+)', result)
...
אם המילה "{word}" נכונה ומתאימה להקשר - השב/השיבי אך ורק במילה אחת:
VALID

אם הילד/ה התכוון/ה למילה אחרת - השב/השיבי בפורמט הבא בדיוק, שתי שורות בלבד:
CORRECT: <כאן רק המילה הנכונה שהתכוון/ה אליה, מילה אחת בלבד, בלי שום טקסט נוסף באותה שורה>
TIP: <כאן הטיפ הקצר, בשורה נפרדת - מסביר מתי כותבים כל מילה, מבוסס על המשמעות וההקשר>

חשוב מאוד: בשורת CORRECT יש לכתוב אך ורק את המילה הנכונה עצמה, ללא הסברים. ההסבר שייך אך ורק לשורת TIP.
...
💎 טיפ: הסבר קצר וברור. הטיפ צריך להיות פשוט, מוחשי, וכזה שילד יתחבר אליו. השתמש/י בקשר ויזואלי או רגשי למילה עצמה - למשל "מכולת היא מקום שמכיל הכל - ולכן עם כ'" או "בתוך טיול יש תמיד טבע - ולכן עם ט'". אל תשתמש/י במינוח לשוני יבש כמו שורש או בניין. משפט אחד קצר שנשאר בזיכרון.
